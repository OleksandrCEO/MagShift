...
MagShift слухає клавіатуру і пам'ятає останню набрану фразу (до 20 клавіш; пам'ять скидається після 1 секунди тиші,
на `Enter`/`Tab`/`Esc` або після комбінацій на кшталт `Ctrl+C`). На подвійний `Shift` він стирає фразу через
`Backspace`, перемикає розкладку і набирає фразу знову. Буфер обміну не використовується, нічого нікуди не
надсилається.
...
Інсталятор ставить `python3-evdev`, копіює програму в `/usr/local/bin/magshift` і додає udev-правила, тож жодних
груп `input`/`uinput` налаштовувати не треба. Якщо одразу не запрацювало, вийдіть із сесії та зайдіть знову.
