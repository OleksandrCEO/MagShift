...
# This script installs SkySwitcher to /usr/local/bin and configures udev rules
...
echo ""
echo "[2/4] Installing SkySwitcher to $INSTALL_DIR/$SCRIPT_NAME..."
...
cat > "$RULE_FILE" <<EOF
# SkySwitcher udev rules - Grant access to input devices for active user
...
echo "2. Run 'skyswitcher' to start the program"
echo "3. To autostart on login, add 'skyswitcher' to your DE's autostart"
echo ""
echo "Options:"
echo "  skyswitcher           # Start with default Meta+Space hotkey"
echo "  skyswitcher -k alt    # Use Alt+Shift instead"
